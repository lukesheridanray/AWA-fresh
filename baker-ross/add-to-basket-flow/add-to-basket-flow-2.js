...
exp.log('Add to basket flow v2 - 1.0');
...
exp.func.successDropdown = function() {
    ...
    var freeDelivery = cartContents.find('.free-delivery');

    // Hide free delivery initially
    freeDelivery.hide();

    /**
     * If the "not eligible" element is hidden, then they are eligible, so show
     * the complete free-delivery block.
     */
    if (freeDelivery.find('.free-delivery-eligible-not').css('display') === 'none') {
        freeDelivery.show();
    }
...
exp.func.addToBasket = function(e, $this, type) {
    ...
    exp.func.mutateListener( $('.top-cart'), exp.func.successDropdown );
