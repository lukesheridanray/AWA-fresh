            ...
            switch (method) {
                case 'html+callback':
                    ...
                    box.load(desturl + ' ' + opts.itemSelector, undefined, function infscr_ajax_callback(responseText) {
                        if( $( responseText ).find('.products-grid:last .item:last').html() == $('.products-grid:last .item:last').html() ) {
                            instance._error('end');
                        } else {
...
exp.init = function() {
    ...
    $('.toolbar:eq(1), .toolbar:eq(1) .limiter').remove();

    $('.toolbar:eq(0)').html(
        $('.toolbar:eq(0)').html().replace(/(.*)(of)(.*)(<\/p>)/, function(p1,p2,p3,p4,p5) {
