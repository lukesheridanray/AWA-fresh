/* REMOVE */
$('BODY > DIV#Container:first-child').css({"display":"none"});
/* CUSTOM CODE */
setTimeout(function(){
  
  $('BODY > DIV#Container:first-child').css({'display':'block'});
  ...
  //}
	...
	exp.init = function() {
		...
		if (window.location.pathname == "/") {
			...
			$(".CompareButton").hide();
			$(".ProductCompareButton").change(function() {
			...
			$('input:checkbox[value="white"]').change(function() {
				if ($(this).is(':checked')) {
			...
			$('input:checkbox[value="remote-complete').change(function() {
				if ($(this).is(':checked')) {
					$('.ProductList').find('li').each(function() {
			...
			$('input:checkbox[value="LED Option"]').change(function() {
				if ($(this).is(':checked')) {
					$('.ProductList').find('li').each(function() {
						var category = $(this).find('.AWA-almafi-title').text();
			...
			$('input:checkbox[value="remote-compatible"]').change(function() {
				if ($(this).is(':checked')) {
					$('.ProductList').find('li').each(function() {
						var category = $(this).find('.AWA-almafi-title').text();
			...
			$('input:checkbox[value="energy saving"]').change(function() {
				if ($(this).is(':checked')) {
					$('.ProductList').find('li').each(function() {
						var category = $(this).find('.AWA-almafi-title').text();
			...
			$('input:checkbox[value="0-136"]').change(function() {
				if ($(this).is(':checked')) {
					$('.ProductList').find('li').each(function() {
			...
			$('input:checkbox[value="137-205"]').change(function() {
				if ($(this).is(':checked')) {
					$('.ProductList').find('li').each(function() {
						var price = $(this).find('.AWA-price').text().slice(1);
			...
			$('input:checkbox[value="206-273"]').change(function() {
				if ($(this).is(':checked')) {
					$('.ProductList').find('li').each(function() {
						var price = $(this).find('.AWA-price').text().slice(1);
			...
			$('input:checkbox[value="274-342"]').change(function() {
				if ($(this).is(':checked')) {
					$('.ProductList').find('li').each(function() {
						var price = $(this).find('.AWA-price').text().slice(1);
			...
			$('input:checkbox[value="343-410"]').change(function() {
				if ($(this).is(':checked')) {
					$('.ProductList').find('li').each(function() {
						var price = $(this).find('.AWA-price').text().slice(1);
		...
		switch(url) {
		    ...
		    case "http://www.theceilingfancompany.co.uk/led-bulbs/":
		    	insertCategoryTitleAndKeepTitles();
		    	break;
		    default:
		        //default code block
		}
		...
		function insertCategoryTitleAndKeepTitles() {
			var categoryTitle = $('.Breadcrumb').find('li:not(:has(a))').text();
			$('#AWA-title-holder').text(categoryTitle);
			$('#AWA-title-holder').attr('style', 'margin-left: 2px;');

			// Make product boxes larger
			$(".Content .ProductList li").attr('style', 'height: 350px !important');
		}
		...
		if ($('.ProductMain').length > 0) {
			...
			$('.VariationProductPrice').bind("DOMSubtreeModified",function(){
				...
				if (window.location.pathname == "/vienna-white/" || window.location.pathname == "/vienna-stainless-steel/" || window.location.pathname == "/fantasia-vienna-ceiling-fan-polished-brass-42/") {
					$('input:radio[value=105]').change(function(e) {
				        if ($(this).is(':checked')) {
				        	setTimeout(function() {
					...
					$('input:radio[value=120]').change(function(e) {
				        if ($(this).is(':checked')) {
				        	setTimeout(function() {
			...
			$('.AWA-options a').click(function(e) {
				e.stopPropagation();
				e.preventDefault();
				var image = $(this).next();
				image.show();
