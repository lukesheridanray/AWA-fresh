	...
	*/
	...
	exp.init = function() {
	    ...
	    if (window.location.pathname == "/") {
			...
			$('.product-title').find('strong').css('visibility', 'hidden');
			...
			// We need to get the product code from inside a script tag if on mobile
			var pCodeText = $('script:contains("product_code")').text();
			var pCodeTrimmed = pCodeText.substring(pCodeText.indexOf("product_code"));
			console.log(pCodeTrimmed);
			var pCodeTrimmed2 = pCodeTrimmed.substring(15);
			var productCode = pCodeTrimmed2.substring(0, 6);

			if (productCode.substring(0, 2) == "11") {
				$('.productAttributeValue').last().append("<div style='clear:bother'></div><div id='show-dr'><a id='add-dr' href='#'>Add drop rod?</a></div>");
				...
				if (newTitle == "Zeta" || newTitle == "Commercial") {
					$('#add-dr').remove();
				}


				if (dropRods[newTitle]) {
					var dropList = "<ul class='AWA-drop-list'>";
					if (dropRods[newTitle]["twelve"]) {
						dropList += "<li><a class='AWA-drop' href='" + dropRods[newTitle]["twelve"] + "'>Add 12\" drop rod (&pound;11.25)</a></li>";
					}
					if (dropRods[newTitle]["eighteen"]) {
						dropList += "<li><a class='AWA-drop' href='" + dropRods[newTitle]["eighteen"] + "'>Add 18\" drop rod (&pound;13.95)</a></li>";
					}
					if (dropRods[newTitle]["thrity"]) {
						dropList += "<li><a class='AWA-drop' href='" + dropRods[newTitle]["thrity"] + "'>Add 30\" drop rod (&pound;22.05)</a></li>";
					}
					if (dropRods[newTitle]["fourtyEight"]) {
						dropList += "<li><a class='AWA-drop' href='" + dropRods[newTitle]["fourtyEight"] + "'>Add 48\" drop rod (&pound;30.60)</a></li>";
					}
					if (dropRods[newTitle]["seventyTwo"]) {
						dropList += "<li><a class='AWA-drop' href='" + dropRods[newTitle]["seventyTwo"] + "'>Add 72\" drop rod (&pound;37.80)</a></li>";
					}
					if (dropRods[newTitle]["kit"]) {
						if (dropRods[newTitle]["kit"] == "http://www.theceilingfancompany.co.uk/conversion-kit/") {
							$('#show-dr').after("<div id='show-ck'><div style='clear:both'></div><a id='add-ck' href='" + dropRods[newTitle]["kit"] +  "'>Add conversion kit?</a></div>");
						} else {
							$('#show-dr').after("<div id='show-ck'><div style='clear:both'></div><a id='add-ck' href='" + dropRods[newTitle]["kit"] +  "'>Add flush mount kit?</a></div>");
						}
			...
			$('.AWA-drop').click(function(e) {
				...
				$.ajax({
				    ...
				    success: function(data) {
				    	...
				    	$('#add-dr').after(dropRodForm);

				    	$('#productDetailsAddToCartForm2 select').attr('style', 'visibility: visible');
			 	    	$('#productDetailsAddToCartForm2 .name:contains("Colour")').text("Drop Rod Colour:");
