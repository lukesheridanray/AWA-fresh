	...
	exp.init = function() {
		...
		if ($('.ProductMain').length > 0) {
			...
			function populateFeatures() {
			    ...
			    $('#ProductDetails .BlockContent').attr('style', 'border: solid 1px lightgray; float: right; width: 350px;');
			...
			if ($.trim($('.VariationProductSKU').text()).substring(0, 2) == "11") {
				$('.BulkDiscountLink').after("<div style='clear:bother'></div><div id='show-dr'><a id='add-dr' href='#'>Add drop rod? (optional)</a></div>");
				...
				if (dropRods[fanCategory]) {
					...
					if (dropRods[fanCategory]["kit"]) {
						dropList += "<li><a class='AWA-drop' href='" + dropRods[fanCategory]["kit"] + "'>Add mounting kit (&pound;13.95)</a></li>";
					}

					dropList += "</ul>";
					console.info(dropList)
			...
			$('.AWA-drop').click(function(e) {
				...
				$.ajax({
				    ...
				    success: function(data) {
				    	...
				    	$('#productDetailsAddToCartForm .BulkDiscount').first().find('input').remove();
				    	...
				    	$('#productDetailsAddToCartForm2').append($('#AWA-warranty'));
				    	...
				    	var dropRodPrice = response.find('.DetailRow.PriceRow .Value .VariationProductPrice').text();
				    	console.log(dropRodPrice);


				    	// Formatting
				    	$('#productDetailsAddToCartForm .Label:contains("Fan Quantity:")').parent().attr('style', 'padding: 0');
				    	$('#productDetailsAddToCartForm .Label:contains("Fan Quantity:")').attr('style', 'width: 104px; color: green;');
				    	$('#productDetailsAddToCartForm2 .productAttributeList .productAttributeLabel').attr('style', 'margin-left: -142px !important;');
				    	$('#productDetailsAddToCartForm2').find('input[type=image]').before("<div style='clear: both;'></div>");
				    	$('#productDetailsAddToCartForm2').find('input[type=image]').attr('style', 'margin-left: 155px; margin-top: 18px;');
