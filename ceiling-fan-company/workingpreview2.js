  ...
  //}
	...
	exp.init = function() {
		...
		if ($('.ProductMain').length > 0) {
			...
			if ($.trim($('.VariationProductSKU').text()).substring(0, 2) == "11" || fanCategory == "Omega") {
				...
				$.ajax({
				    ...
				    success: function(data) {
				    	...
				    	if ($('.dummy-buy').length < 1) {
					    	...
					    	$('.dummy-buy').before('<div style="clear: both"></div>');
			...
			$('#ProductDetails form').submit(function (e) {
				...
				if (!$('#productDetailsAddToCartForm2').length && !$('#productDetailsAddToCartForm3').length) {// Only fan
					$.ajax({
						type: "POST",
						url: "http://www.theceilingfancompany.co.uk/cart.php",
						//data: $(this).serialize(), // serializes the form's elements.
						success: function(data) {
							console.info('submtted ONLY form 1');
							$(this).remove();
							$('#new-loading').hide();
						}
					});
				} else if ($('#productDetailsAddToCartForm2').length && !$('#productDetailsAddToCartForm3').length) {// Only fan and DR
					$.ajax({
						type: "POST",
						async: false,
						url: "http://www.theceilingfancompany.co.uk/cart.php",
						data: $('#productDetailsAddToCartForm2').serialize(), // serializes the form's elements.
						success: function(data) {
							console.info('SECOND CASE: submtted form 2');
							$('#productDetailsAddToCartForm2').remove();
								$.ajax({
								type: "POST",
								url: "http://www.theceilingfancompany.co.uk/cart.php",
								//data: $(this).serialize(), // serializes the form's elements.
								success: function(data) {
									console.info('SECOND CASE: submtted form 1');
									$(this).remove();
									$('#new-loading').hide();
								}
							});
				...
				} else if (!$('#productDetailsAddToCartForm2').length && $('#productDetailsAddToCartForm3').length) {// Only fan and CK
					$.ajax({
						type: "POST",
						async: false,
						url: "http://www.theceilingfancompany.co.uk/cart.php",
						data: $('#productDetailsAddToCartForm3').serialize(), // serializes the form's elements.
						success: function(data) {
							console.info('THIRD CASE FROM 3');
							$('#productDetailsAddToCartForm3').remove();
					...
					// NOT WORKING IN IE
				} else if ($('#productDetailsAddToCartForm2').length && $('#productDetailsAddToCartForm3').length) {// All 3
					$.ajax({
						type: "POST",
						async: false,
						url: "http://www.theceilingfancompany.co.uk/cart.php",
						data: $('#productDetailsAddToCartForm2').serialize(), 
						success: function(data) {
							console.info('3 submtted form 2');
							//$('#productDetailsAddToCartForm2').remove();
							...
							async: false,
							url: "http://www.theceilingfancompany.co.uk/cart.php",
							data: $('#productDetailsAddToCartForm3').serialize(), 
								success: function(data) {
									console.info('3 submtted form 3');
									//$('#productDetailsAddToCartForm3').remove();
									...
									$.ajax({
										...
										//data: $(this).serialize(),
										success: function(data) {
											console.info('3 submtted form 1');
											$('#new-loading').hide();
										}
									});
								}
