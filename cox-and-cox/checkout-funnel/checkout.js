...
exp.init = function() {
    ...
    var heightSetter = setInterval(function () {
        if (heightSetCount++ < 20) {
            $('#checkoutSteps').css('height','960px');
        }
        else {
    ...
    // Step 2 - Billing - no address book
    // 
    if ( ! $('#billing-address-select').length) {
        ...
        // Step 2.5 - Delivery Address - no address book
        //
    ...
    //
    // Step 2 and 2.5 - with address book
    // 
    // 
    // 
    // 
    ...
    if ($('#billing-address-select').length) {
        ...
        $("#co-billing-form > fieldset:nth-child(1) > ul:nth-child(1) > li:nth-child(2)").hide();
        ...
        $("#co-billing-form .input-box").css('margin-right','70px');
        $("#co-billing-form label.required").css('width', '90px');
        $("#billing-new-address-form > fieldset > ul > li:nth-child(2) > div > label").css('width', '90px');
        ...
        // Align select box correctly
        $('#co-billing-form > fieldset > ul > li.wide > .input-box').css('margin-right', '180px');
        $('#billing-address-select').css('margin-bottom', '.5em');

        // Sort out continue button
        $("#continue-btn span").remove();
        $("#continue-btn").text('Continue');
        $("#continue-btn").attr('style','font-size: 1.5em; font-weight: bold; padding: 0.75em 2em; margin: 0px auto; border: 1px solid rgb(0, 0, 0); background: none repeat scroll 0% 0% rgb(241, 194, 0);');
        $("#co-billing-form > fieldset > ul").append('<li style="margin-top:2em;" id="billing-buttons-container2"></li>');
        var continueBtn = $("#continue-btn").detach();
        $("#billing-buttons-container").append(continueBtn);
        $("#billing-buttons-container").css('text-align', 'center');
        $("#billing-buttons-container button").css('float', 'none');

        // Remove save address book link
        $("#billing-new-address-form > fieldset > ul > li.control").hide();
        // Fix labels with extra :
        $("#co-billing-form > fieldset > ul > li:nth-child(4) > label").text('Deliver to this address');
        $("#co-billing-form > fieldset > ul > li:nth-child(5) > label").text('Deliver to different address');
        $("#co-billing-form > fieldset > ul > li.wide > label").text('Select a billing address from your address book or enter a new address');
        ...
        // Spacing around save address tickbox
        $("#billing-new-address-form > fieldset > ul > li.control").css('margin-top', '1.5em');
        $("#co-billing-form > fieldset > ul > li:nth-child(4)").css('margin-top', '1em');
        ...
        // Underline links
        $('#meanbee\\:billing_input_address_manually').css('text-decoration', 'underline');
        $('#meanbee\\:billing_show_another_link').css('text-decoration', 'underline');

        // Spacing under manual entry link
        $('#billing-new-address-form > fieldset > ul > li:nth-child(10) > div').css('margin-top', '1em');
    ...
    // 
    // 2.5 with address book
    // 
    // 
    if ($('#shipping-address-select').length) {

        // Align select box correctly
        exp.func.waitForElement(function() {
            $('#co-shipping-form > fieldset > div:nth-child(2) > ul > li.wide > div').css('margin-right', '180px');
            $('#shipping-address-select').css('margin-bottom', '.5em');

            // Use billign address to the left
            $("#co-shipping-form > fieldset > div:nth-child(2) > ul > li.control").css('margin-left', '0');
            $("#co-shipping-form > fieldset > div:nth-child(2) > ul > li.control").css('margin-top', '1em');

            $("#gift-message > li > div").css('float', 'none');
        }, 60000, 1000);

        // Don't save to address book
        $('#save-address-book').hide();
        ...
        // Spacing under manual entry link
        $('#shipping-new-address-form > fieldset > ul > li:nth-child(10) > div').css('margin-top', '1em');
        ...
        $("#shipping-buttons-container").css('text-align', 'center');
        $("#shipping-buttons-container button").css('float', 'none');
