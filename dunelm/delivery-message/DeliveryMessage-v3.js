...
var exp = (function($) {
	...
	*/
	...
	exp.init = function() {
		...
		// ajaxSuccess is not called a second time if the product is changed back to an earlier loaded size
		...
		$(".pdp-input-wrap-quantity .pretend-dd").click(function(){
			setTimeout(function() {
				checkDelivery();
			}, 500);
		});
		...
		function checkDelivery() {
			...
			var quantity = $(".pdp-input-wrap-quantity #quantity option:selected").text().match(/\(([^()]+)\)/)[1];
			...
			if (range == true) {
				...
				exp.log("Price range detected - not updating shipping");
			} else if (heavy) {
				exp.log('Heavy item in cart - not changing delivery');
				return;
			} else if (price >= 49) {
				$("[id^='standardDeliveryTxt-']").find("strong:contains('£3.95')").text("FREE");
				exp.log('Changing delivery');
			} else {
				exp.log("Not updating shipping - low price or other condition");
			}
		}
