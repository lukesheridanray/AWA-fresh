...
var exp = (function(vwo_$) {
	...
	*/
	exp.condition = vwo_$('#dropdown_menu');
	...
	exp.init = function() {
	    ...
	    vwo_$('head').append('<style type="text/css">'+this.css2+'</style>');
		...
		function mobileHomeGiftFinder() {
			// Change header
    		vwo_$("#ctl00_dvMobileGiftFinderHeader").text("Find a gift for a child aged");
    		vwo_$(".mobile-gift-finder-header").attr('style', 'background: #922C83; font-size: 1.5em');
    		vwo_$("#ctl00_dvMobileGiftFinder").find(".block").last().hide();

    		// Format checkboxes
    		vwo_$("#ctl00_dvMobileGiftFinder").find('strong').hide();
    		vwo_$("#ctl00_dvMobileGiftFinder").find(".block").first().attr("style", "width: 100%");
    		vwo_$(".mobile-gift-finder .checkbox").attr("style", "width: 163px");
    		vwo_$(".mobile-gift-finder label").attr("style", "font-size: 17px; font-weight: bold;");

    		// Style button
    		vwo_$(".mobile-gift-finder-button a").attr('style', 'background: #AA28F5; font-size: 14px; width: 168px;');
    		vwo_$(".mobile-gift-finder-button").attr('style', 'margin-top: 0;');

    		// Style container
    		vwo_$(".mobile-gift-finder .container").attr("style", "height: 66px; margin-top: 0");
    		vwo_$(".mobile-gift-finder").attr("style", "height: 118px;");

			// Add value proposition
			vwo_$(".mobile-gift-finder").after(exp.vars.valueProp);

			// Add reviews
			vwo_$('#AWA-value-prop').after(exp.vars.reviews);

			// Remove slider image
			vwo_$('#slideshowmain').hide();
			vwo_$("#ctl00_cphMaster_dvMobilePanel").hide();

			// Move shipping & payment banners up to the top
			vwo_$(".header").after(vwo_$('#ctl00_cphMaster_vsNewsLetterSignUp'), vwo_$("#ctl00_cphMaster_imgBanner1"), vwo_$("#ctl00_cphMaster_imgBanner2"));
		}
	    ...
	    if (window.location.pathname == "/") {
	    	...
	    	if (vwo_$("#ctl00_dvMobileGiftFinder").length) {
	    	...
	    	// Desktop home
		    	...
		    	vwo_$('.primary-wrapper').prepend(vwo_$('.gift-finder'));

			    // Remove old gift guide header
			    vwo_$('.gift-finder-header').remove();

			    // Remove unused inputs
				vwo_$('.gift-finder li:nth-child(1), .gift-finder li:nth-child(2), .gift-finder li:nth-child(4), .gift-finder li:nth-child(5), .gift-finder li:nth-child(6), .gift-finder li:nth-child(7)').remove()
				
				// Format search button
				vwo_$('.gift-finder').find('.submit-button').text("Find the perfect gift");
				vwo_$('.gift-finder').find('.submit-button').attr('style', 'background: #AA28F5; width: 137px;');

				// Add new header to gift guide
				vwo_$('.gift-finder').before("<div id='AWA-header'><h1>Find a gift for a child aged</h1></div>");

				// Add value proposition
				vwo_$('.gift-finder').after(exp.vars.valueProp);

				// Add reviews
				vwo_$('#AWA-value-prop').after(exp.vars.reviews);

				// Remove slider image
				vwo_$('#slideshowmain').remove();
	    	}
	    } else {
			...
			if (vwo_$('.main_internal_box').length) {
				vwo_$('.main_internal_box').prepend(exp.vars.new_gift_guide);
			} else if (vwo_$('.main_internal_full_box').length) {
				vwo_$('.main_internal_full_box').prepend(exp.vars.new_gift_guide);
			}
			
			// Hide skill row
			vwo_$('.AWA_HPC_gift_guide_col').last().hide()
			// Remove santa hat
			vwo_$('.AWA_HPC_gift_guide_title').find('img').remove();
		    ...
		    vwo_$.get('/default.aspx', function(home_page_data){
		    ...
		    vwo_$('#AWA_HPC_gift_guide_submit').on('click', function(e){
		        e.preventDefault();
