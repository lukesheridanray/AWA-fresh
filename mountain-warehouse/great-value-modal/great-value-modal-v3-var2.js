...
var exp = function($) {
	...
	exp.log('Great Value Modal - v3 - var2');
	...
	*/
	...
	exp.init = function() {
		...
		if ($(".size-list").length) {
			exp.func.waitFor(function() { return $(".size-list").find("li").find("input:radio:enabled").length}, activate);
			exp.log("activate");
		}


		function activate() {
			// Obtain price of item
			var price;
			if ($("#nowprice").find('span[itemprop="price"]').length) {
				price = Number($("#nowprice").find('span[itemprop="price"]').first().text().match(/[\$\£\€](\d+(?:\.\d{1,2})?)/)[1]);
			} else {
				price = Number($(".now").first().find('span[itemprop="price"]').first().text().match(/[\$\£\€](\d+(?:\.\d{1,2})?)/)[1]);
			}

			/* --- EXCLUDE FOR PRODUCTS OVER £X --- */
			if (price >= 80) {
				exp.log("Product excluded from Great Value Modal");
				return;
			}
			...
			exp.log($("#nowprice").find('span[itemprop="price"]').length);
			exp.log($(".now").first().find('span[itemprop="price"]').length);
			if ($("#nowprice").find('span[itemprop="price"]').length) {
