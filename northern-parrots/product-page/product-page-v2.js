...
jQuery(".leftNAVIGATION,.navtopround,.navbottomround,.productTITLE,#crumb").remove();
...
// Move "More details" besides short desc to a new line, and add a little bit of padding.
jQuery(".prodSHORTdesc > a").before("<br/>");

jQuery("#Form1 tbody > tr:eq(0) > td:eq(0)").addClass("split-test-option-select");

jQuery(".wishlist a").appendTo(".prodMessage > ul > li:eq(2)");

jQuery(".prodMessage > ul > li:eq(0)").replaceWith("<li class=\"split-text-delivery-info\"><a href=\"javascript:showPopupA('cs_delivery_summary');\">Delivery info</a></li><li class=\"split-test-ship-timer\"><br />"+"Free delivery for all orders over &pound;49</li>");
...
/*
jQuery('.split-text-delivery-info a').on('click',function(e){
    e.preventDefault();
    var overlayCallback = function(){
        $('#ajaxEditorial').append("<div class=\"ajaxArticleHEADER\"><div style=\"float:right\"><a href=\"javascript:hidePopup();\"><img src=\"/images/core/close.gif\" alt=\"x\"></a>	</div>DELIVERY SUMMARY</div>\n\n<div class=\"ajaxArticleTEXT\"><h2> \
	Free Next Day UK Delivery – Orders Over £99</h2><p>\
	Place your order online before 2pm Monday to Friday and choose Next Day delivery, and we’ll deliver your order by Parcelforce the very next working day.</p><p>\
	If your order is less than £99, next day delivery costs just £5.99.</p><p>\
	Available for in stock orders (excluding some large items) and to most UK mainland addresses.<br>\
	&nbsp;</p><h2>\
	Free Standard UK Delivery – Orders Over £49</h2><p>\
	Standard Delivery normally takes 3-5 working days and your parcel will usually arrive by either Royal Mail or Parcelforce, depending on the size and weight of the order.</p><p>\
	If your order is less than £49, Standard Delivery costs just £3.99.</p><p>\
	We can also offer other delivery options including<br>\
	•Next Working Day AM - £7.99<br>\
	•Saturday AM - £9.99</p><p>\
	Please note, the expected delivery times above exclude Saturdays, Sundays and Bank Holidays.<br><br>\
	Large items such as Cages and some Stands are only available for delivery to UK Mainland addresses and may take an extra few days or be subject to surcharge in some areas.</p><p>\
	For more information on delivery charges and speeds to the UK and International addresses, and any restrictions, please see our full <a href=\"http://www.northernparrots.com/delivery-information-artlcs_delivery/\">Delivery Information</a></p>\
<p><br /><em>Reliable and almost always next day delivery. Well packed. Worth the p&amp;p as you get premier delivery.</em> - Elizabeth Jeffs, regular customer, 14 April 2014</p> \
<p><br /><em>I know that when I replenish supplies from Northern Parrots, the order will be speedily completed and on my doorstep earlier than I could reasonably expect.</em> - Mike Staveley, 1 April 2014</p> \
</div></div>" );
        $('#ajaxEditorial').css({top:($(document).scrollTop() + 200 + 'px'), left:50 +'%', margin: '0 0 0 -' + ($('#ajaxEditorial').width() / 2) + 'px', display:'block'}); 
    }
    showOverlayBoxdesign(overlayCallback);
...
// wait for and populate ratings widget
function split_test_ratings() {
    var ratings_wrap = jQuery('<div class="split-test-ratings"></div>');
    jQuery("#productAverageRating .stars-large").appendTo(ratings_wrap);
    jQuery("#product_review_tandc_links a:eq(0)").appendTo(ratings_wrap);
    var new_string = ratings_wrap.html().replace('Read all ','(', 'gi');
    ratings_wrap.html(new_string);
    var new_string_new = ratings_wrap.html().replace('reviews','reviews)', 'gi').replace('#Commentreviews)','#CommentReviewsContainer', 'gi');
    ratings_wrap.html(new_string_new);
    jQuery("#prodDETAILS").prepend(ratings_wrap);
}
wait_for_element('.commenttext', split_test_ratings);
...
$(".prodOPTIONS tbody > tr:eq(3) > td:eq(2), .prodOPTIONS tbody > tr:eq(3) > td:eq(0)").remove();
$(".prodOPTIONS tbody > tr:eq(3) > td:eq(0)").attr('colspan','4');
...
$prMatch = jQuery('.prmatch');
if ($prMatch.length === 1)
{
    // Reposition and flavour text
    $prMatch.insertBefore('div.prodOPTIONS');
    $prMatch.append('<p>This item is guaranteed to be<br/> the lowest price on the web.</p>');
