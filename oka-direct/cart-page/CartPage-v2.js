	...
	*/
	...
	exp.init = function() {
		...
		function formatPricing() {
			// If there is a quantity greater than 1 for a product then show the "price each" in the price column
			$(".table-striped").find("tr:not('.upper, .t-sub, .t-delivery, .t-total')").each(function() {
				var quantity = $(this).find("select").val();
				if (quantity > 1) {
					var priceEach = $.trim($(this).find("span:contains('Delivery:')").text()).match(/£.+/)[0];
					var priceEachText = "(" + priceEach + " each)";
					$(this).find(".t-price").html($(this).find(".t-price").html() + "<br><span class='AWA-small'>" + priceEachText + "</span>");
				}
			});

			// Remove "Price Each" from every product
		...
		// Rerun functions when a quantity is changed
