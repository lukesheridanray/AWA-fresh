...
exp.init = function() {
    ...
    function getImage() {
        $(".basket-items table:first tr td img").closest("a").each(function() {

            // Get product code
            var productCode = $(this).attr("href");
            productCode = productCode.substr(productCode.lastIndexOf('/') + 1);

            if (basketImageData[productCode]) {
                // Replace iamge
                $(this).find("img").attr('src', 'http://www.tandmpics.com/pictures/tmuk/_230/' + basketImageData[productCode]);
            }
