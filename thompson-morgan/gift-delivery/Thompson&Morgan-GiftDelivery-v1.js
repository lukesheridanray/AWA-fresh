...
var exp = (function($) {
	...
	*/
	...
	exp.init = function() {
		/* NOTE: This experiment cannot be URL targeted because we need to modify elements on the basket page, gift delivery options page, and delivery & payment page. If you make edits on these pages then the URL changes and there is no common string between these page URLs. */

	    // Append style to head
	    $('head').append('<style type="text/css">'+this.css+'</style>');

	    // Align gift notification div on basket page next to "Your savings" div from T&M Basket page experiment
	    if ($('#awa-savings-box').length) {
		    $('.heading-basket').attr('style', 'float: left; width: 680px;');
		    $('.basket-portlet .giftSku').attr('style', 'float: right; margin-left: 0; margin-bottom: 10px;');
		    $('#awa-savings-box').parent().after("<div style='clear: both'></div>");
	    }
