...
var exp = (function($) {
	...
	*/
	...
	exp.init = function() {
	    ...
	    // Select "in time for Christmas" if that's the only delivery option
	    if ($(".delivery-dates-options").first().find("input:radio").length == 1) {
	    	$(".delivery-dates-options").first().find("input:radio.christmas").attr('checked','checked');
	    }
	    ...
	    save.hide();
	    $(".giftAddressPersonalize").append("<div id='AWA-save-message'>Please complete all three steps. A ‘Save’ button will then appear here to save your delivery address, delivery date and personal message for this gift item.</div>");
	    var saveMessage = $("#AWA-save-message");
	 	saveMessage.hide();
	    if ($('.giftAddressPersonalize').length) {
	    	setInterval(function() {
	    		var messagePresent = $('.no-personal-message').attr('checked') || $('.personal-message-line').val() != "";
	    		if ($('.dateRadio').find('input').filter(':checked').length && $('.payment_address_select').val() != "" &&  messagePresent) {
	    			save.show();
	    			saveMessage.hide();
	    		} else {
	    			save.hide();
		...
		exp.log('Before setTimeout:');
	    exp.log($('#new-customer').length);
	    exp.log($.trim($('#new-customer').find('h3').text()));
	  	// Delay address moday check by 1.5 seconds since sometimes the #new-customer div is loaded late
	  	setTimeout(function() {
