	...
	*/
	...
	exp.init = function() {
	    ...
	    function step2() {
	    	...
	    	$(".userDetails").find('a[name=ADDRESS_FIELDS]').before("<div id='AWA-manual'><a id='AWA-manual-link' href='#'><span class='AWA-underline'>Click here</span> to enter address manually</a></div>");

	    	$("#AWA-manual-link").click(function(e) {
	    		e.preventDefault();
	    		var fakeReg = $("#registration_submit").clone();
	    		fakeReg.attr('id', 'AWA-fake-reg');

	    		$("#AWA-manual").hide();
	    		middleForm.show();
	    		emailPhone.hide();

	    		$("#registration_submit").before(fakeReg);
	    		$("#AWA-fake-reg").show();

	    		$("#AWA-fake-reg").click(function(e) {
	    			e.preventDefault();
	    			subscriptions.show();
			    	emailPhone.show();
