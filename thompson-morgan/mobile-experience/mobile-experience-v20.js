...
var exp = (function($) {
	...
	exp.vars = {
		...
		row1: 	"<div id='AWA-row1'>\
					...
					<div id='AWA-m-cart'>\
						...
						<div id='AWA-co-text'><a href='/basket'>Checkout</a></div>\
					</div>\
		...
		homepage: 	"<div id='AWA-homepage'>\
						<div id='AWA-h-hero'><a href='/garden-ready-plants-summer'><img src='/static-images/tandm/homepage/banners/garden-ready-hp-feb16.jpg'></a></div>\
	...
	/* --- MOBILE NAVIATION --- */
		...
		// If there are any items in the basket then show "Checkout" link
		if ($('#AWA-item-num').text() !== "") {
			$("#AWA-co-text").show();
		}
		...
		/* --- LISTING PAGE --- */
		if ($('#268917436').length && window.location.pathname.indexOf("search") == -1) {
			exp.log("Modifying listing page...");
			this.listingCSS = '\
				#AWA-listing-page {\
					font-size: 170%;\
					padding: 15px;\
    			...
    			.AWA-l-stock select {\
   					...
   					border: solid 1px black;\
   					background: url(https://raw.githubusercontent.com/doctafaustus/awa/master/arrow.jpg) no-repeat right;\
    			}';
			...
			$(".AWA-l-stock select").change(function() {
				...
				var sku = $(this).parent().parent().find(".stockInfo").filter(function() {
				    return $(this).find('.size').text() === selectedSKU;
				});
				sku.show();

				var despatchText = sku.find('.despatch').text();
				$(this).parent().find(".AWA-l-despatch").text(despatchText);
			});
			...
			$(".addToBasket").attr('style', 'background-color: #B60718; background-image: none; text-indent: 0px; color: white; width: 100%; height: 120px !important; font-size: 38px; text-align: center;');
			...
			// Increase size of star ratings
			$('.resultSet').find('img[src$="/5-star.jpg"], img[src$="/4-star.jpg"], img[src$="/3-star.jpg"], img[src$="/2-star.jpg, img[src$="/1-star.jpg"]"]').attr('style', 'width: 20%; margin-right: 10px;');


		    // Insert confirmation modal
			$('<div id="__msg_overlayZ">').css({
			      "width" : "100%"
			    , "height" : "100%"
			    , "background" : "#000"
			    , "position" : "fixed"
			...
			$('body').append("<div id='AWA-confirmation-modal'>\
				<h2>Item added to your cart</h2>\
				<a href='#' id='AWA-cs'>Continue Shopping</a>\
				<a href='http://www.thompson-morgan.com/basket' id='AWA-co'>Checkout</a>\
			</div>");


			// Event handler for "Continue Shopping" on confirmation modal
			$("#AWA-cs").click(function(e) {
				e.preventDefault();
				$("#__msg_overlayZ").hide();
				$("#AWA-confirmation-modal").hide();
			});

			// If item was added to cart trigger confirmation modal
			if (window.location.pathname.indexOf("/public/QLOnline/list") !== -1) {
				$("#__msg_overlayZ").show();
				$("#AWA-confirmation-modal").show();

				setTimeout(function() {
					$("#__msg_overlayZ").fadeOut();
					$("#AWA-confirmation-modal").fadeOut();
				}, 5000);
			}
			...
			// Insert "Read Description" Link
			$(".AWA-l-stock").before("<a class='AWA-rd' href='#'>Read Description</a>");

			// Remove Useful Links
			$("b:contains('Useful links:')").parent().parent().nextAll().hide().remove();
			$("b:contains('Useful links:')").parent().parent().remove();


			// Toggle product descriptions
			$(".AWA-rd").click(function(e) {
				if ($(this).attr('href') == "#") {
					e.preventDefault();
					var $description = $(this).parent().find('.season').children().show();
					$(".producticon").hide();
					$("b:contains('Customer Rating')").prev('.AWA-unwrapped').hide();
					$(this).text('View Full Product Information').attr('href', $(this).prev().attr('href'));
				} 
			});

			$("b:contains('Customer Rating')").attr('style', 'font-size: 30px;');

			// Add "Make A Selection" for multiple-SKU products
			$(".AWA-l-stock").each(function() {
				if ($(this).find("select option").length > 1) {
					$(this).prepend("<p class='AWA-mas'>Make A Selection</p>");
				}
			});
		...
		if ($('#32485735').length) { // Ensures we're on a product page
			...
			if ($('.no-background-image').text() == "Flowers" || $('.no-background-image').text() == "Vegetables" || $('.no-background-image').text() == "Fruit" || $('.no-background-image').text() == "Large plants") {
				...
				$('.AWA-description a, .AWA-reviews a, .AWA-delivery a, .AWA-grow a').attr('style', 'font-size: 40px;');
		...
		if ($(".heading-basket").length) {
			...
			$(".AWA-m-d").each(function() {
				$(this).append($(this).prev("tr").find(".details").html());
			...
			$("h2:contains('You qualify for special offers')").attr('style', 'margin-top: 150px;');


			var pText = $(".delivery-option").parent().find('dd').first().html();
			$(".delivery-option").parent().find('dd').first().text("");
			$(".delivery-option").html(pText);


		    // Insert basket modal
			$('body').append("<div id='AWA-basket-modal'>\
				<p>Scroll down for checkout</p>\
			</div>");

			setTimeout(function() {
				$("#AWA-basket-modal").fadeOut();
			}, 2000);


			$("#AWA-basket-modal").click(function(e) {
				e.preventDefault();
				$(this).fadeOut();
			});
			...
			$("#removeTable td a img").each(function() {
				...
				var n = url.lastIndexOf('/');
			  	var urlCode = url.substring(n + 1);
			  	$(this).attr('src', imageData[urlCode]);
			  	$(this).attr('style', 'height: 150px; width: 150px;');
			});
			$(".basketPromotions td a img").each(function() {
				...
				var n = url.lastIndexOf('/');
			  	var urlCode = url.substring(n + 1);
			  	$(this).attr('src', imageData[urlCode]);
			  	$(this).attr('style', 'height: 350px; width: 350px;');
			});

			$("#removeDIV a:contains('Remove')").attr('style', 'font-size: 40px !important;').addClass('AWA-remove-circle');
			$(".AWA-remove-circle").html("<img src='//dd6zx4ibq538k.cloudfront.net/static/images/2841/23bba644572e57b9011eabd4a40d1ddd_478_477.png' width='50'>").attr('style', 'margin-left: 14px;');
			$(".AWA-remove-circle").prev('br').remove();

			$(".basket-items").find('p').first().nextUntil('form').hide();
		...
		if ($("h1:contains('1. Billing address details')").length) {
			...
			// Format credit card details area
			$(".card-details label").hide();
			$("#checkoutForm3\\:pan").attr('placeholder', 'Card Number');
			$("#checkoutForm3\\:cardholdername").attr('placeholder', 'Cardholder\'s Name');
			$("#checkoutForm3\\:csc").attr('placeholder', 'CSC');
			$("a:contains('what is this')").attr('style', 'font-size: 130%');

			$("span.textBold:contains('Click here to')").hide();
			$("#checkoutForm2\\:manualEntry").hide();
			$("#checkoutForm2\\:manualEntry").prev().hide();

			$("h1:contains('2. Delivery address details')").attr('style', 'font-size: 150%;');
			$("#createAddress, #submitOrder").attr('style', 'font-size: 170% !important;');

			$("#checkoutForm1\\:updatedBillingAddress, #checkoutForm1\\:updatedDeliveryAddress").attr('style', 'width: 100%');

			// Hide extra spacing between address fields
			$("#streetName").prev().hide();
			$("#district").prev().hide();
		...
		if (window.location.pathname === "/special-offers") {
			this.specialOffersCSS = '\
				...
				#AWA-offers-list li {\
					...
					background: url(//dd6zx4ibq538k.cloudfront.net/static/images/2841/f07889a894369950c2093a76b9f8adc0_64_78.png) lightgray;\
				    background-size: 25px;\
				    background-repeat: no-repeat;\
				    background-position: 95%;\
				    display: table;\
			...
			$('head').append('<style type="text/css">'+this.specialOffersCSS+'</style>');

			exp.vars.specialOfferVars = {
				...
				mobileWrapper: 	"<div id='AWA-mobile-wrapper'>\
							...
							<ul id='AWA-offers-list'>\
								<li class='AWA-mobile-offer'><a href='http://www.thompson-morgan.com/flower-bulb-sale'>This week's best selling offers</a></li>\
								<li class='AWA-mobile-offer'><a href='http://www.thompson-morgan.com/instant-gardening-range'>Instant Gardening Range - Buy 4, Pay for 3</a></li>\
								<li class='AWA-mobile-offer'><a href='http://www.thompson-morgan.com/garden-ready-plants'>Buy 2 Packs of Garden Ready Plants for just &pound;25</a></li>\
								<li class='AWA-mobile-offer'><a href='http://www.thompson-morgan.com/pre-planted-pots-and-baskets-winter'>Buy any 2 pre-planted pots and baskets for just &pound;24.99</a></li>\
								<li class='AWA-mobile-offer'><a href='http://www.thompson-morgan.com/sale-plant-offers'>Autumn Sale now on!</a></li>\
								<li class='AWA-mobile-offer'><a href='http://www.thompson-morgan.com/autumn-planting-bulbs-inspiration'>4 for 3 on Autumn planting bulbs</a></li>\
								<li class='AWA-mobile-offer'><a href='http://www.thompson-morgan.com/value-seed-varieties'>Great value seeds - Over 240 seed varieties 99p or under</a></li>\
								<li class='AWA-mobile-offer'><a href='http://www.thompson-morgan.com/5for4-seed-offer'>Buy 4 packets of seed get the 5th free!</a></li>\
								<li class='AWA-mobile-offer'><a href='http://www.thompson-morgan.com/fruit/fruit-plants/raspberry-plants/raspberry-full-season-collection/cww3211TM'>Raspberry 'Full Season' Collection - Save up to &pound;19.98</a></li>\
								<li class='AWA-mobile-offer'><a href='http://www.thompson-morgan.com/fruit/fruit-plants/blueberry-plants/blueberry-full-season-collection/cww3308TM'>Blueberry 'Full Season' Collection - Save up to &pound;33.98</a></li>\
								<li class='AWA-mobile-offer'><a href='http://www.thompson-morgan.com/fruit/fruit-plants/strawberry-plants/strawberry-extend-the-season-collection/cww3188TM'>Blueberry 'Full Season' Collection - Save up to &pound;9.98</a></li>\
								<li class='AWA-mobile-offer'><a href='http://www.thompson-morgan.com/vegetables/potatoes/second-early/potato-jazzy/t56501TM'>Potato 'Jazzy' Collection - Just &pound;12.99</a></li>\
								<li class='AWA-mobile-offer'><a href='http://www.thompson-morgan.com/chempak-fertiliser-offer'>Chempak Fertiliser - 3 for 2</a></li>\
								<li class='AWA-mobile-offer'><a href='http://www.thompson-morgan.com/garden-supplies/fertilisers/chempak-incredibloom/t47963TM'>Incredibloom - From just &pound;4.99</a></li>\
								<li class='AWA-mobile-offer'><a href='http://www.thompson-morgan.com/garden-supplies/baskets-and-containers/easy-fill-basket/t47549TM'>Easy Fill Hanging Baskets - From just &pound;14.99</a></li>\
							</ul>\
						</div>"
			};
			...
			$("#12250136").before(exp.vars.specialOfferVars.mobileWrapper);

			// Hide footer
			$("#1853813557").attr('style', 'visibility:hidden;');
		}

		/* --- CATEGORY PAGE * --- */
		if (universal_variable.page) {
			if (universal_variable.page.type === "category") {
				this.categoryCSS = '\
					#AWA-offer-menu-list a {\
					    border-bottom: solid 1px black;\
					    padding: 3.7%;\
					    padding-left: 9%;\
					    background: url(//dd6zx4ibq538k.cloudfront.net/static/images/2841/f07889a894369950c2093a76b9f8adc0_64_78.png) #CAC8C8;\
				...
				$('head').append('<style type="text/css">'+this.categoryCSS+'</style>');

				// Create menu framework
				var offerMenu = "<div id='AWA-offer-menu'>\
									<ul id='AWA-offer-menu-list'>\
									</ul>\
								</div>";

				$("#AWA-row4").after(offerMenu);

				// Get list of categories
				var categories = [];

				// Add in "Take a look at this week's best selling offers" if it exists
				if ($("#CategoryList .azindex-thumb").length) {
					categories.push({ link: $("#CategoryList .azindex-thumb").first().parent().attr("href"), title: "This Week's Best Selling Offers"});
				}

				$(".category-thumb").find("span").each(function() {
					var categoryObject = {};
					categoryObject.title = $(this).text();
					categoryObject.link = $(this).closest("a").attr("href");
				...
				for (var i = 0; i < categories.length; i++) {
					offerHTML += "<a href='" + categories[i].link + "'><li>" + categories[i].title + "</li></a>";
				}

				$("#AWA-offer-menu-list").append(offerHTML);

				// Retain category image
				$("#AWA-offer-menu").before($("#category_image"));

				// Hide rest of page
				$("#UIPortalApplication").hide();
			} else if (universal_variable.page.type === "content" && window.location.pathname.indexOf("garden-supplies") > -1) { // Garden Supplies
					this.categoryCSS = '\
						#AWA-offer-menu-list a {\
						    border-bottom: solid 1px black;\
						    padding: 3.7%;\
					...
					$(".grid_5img").each(function() {
						var categoryObject = {};
						categoryObject.title = $(this).find("img").attr("alt");
						categoryObject.link = $(this).find("a").attr("href");
						categories.push(categoryObject);
					}); 
					exp.log(categories);
