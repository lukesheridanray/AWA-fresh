$.getJSON('https://raw.githubusercontent.com/doctafaustus/hmkb/master/imagedata.json', function(data){
	if(data) {
		imageData = data;
...
var exp = (function($) {
	...
	/* --- MOBILE NAVIATION --- */
		...
		if ($("h2:contains('Login to your account')").length) {
			...
			// Make phone inputs 
			$("#phone").remove();
			$("label:contains('Telephone number * ')").after('<input id="phone" name="phone" type="tel" value="" maxlength="20" size="20" tabindex="13" pattern="\d*">');
			$("#cellphone").remove();
			$("label:contains('Mobile number')").after('<input id="cellphone" name="cellphone" type="tel" value="" maxlength="20" size="20" title="Please enter your best contact number. We will only use this to contact you with queries about your order." tabindex="14" pattern="\d*">');
