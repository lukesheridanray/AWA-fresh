...
var exp = (function($) {
	...
	exp.log('Mobile Product Page v7');
	...
	if ($('.no-background-image').text() == "Flowers" || $('.no-background-image').text() == "Vegetables" || $('.no-background-image').text() == "Fruit" || $('.no-background-image').text() == "Large plants") {
	    ...
	    $('.AWA-description, .AWA-reviews, .AWA-delivery, .AWA-grow').attr('style', 'font-size: 208%;');
	...
	function addTabs(supplies) {
		...
		if (firstReviewSentence == ".") {
			firstReviewSentence = $.trim(firstReview.substring(0, firstReview.indexOf('!')));
			firstReviewSentence += "!";
		}
		if (firstReviewSentence == ".") {
