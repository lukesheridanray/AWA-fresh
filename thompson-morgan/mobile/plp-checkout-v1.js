...
var exp = (function($) {
    ...
    exp.init = function() {
        ...
        //Make the price on same line as description
        function makePriceFloatLeft(){
        $('.productInfoLeft').find('ul:first').find('.size').unwrap();
        jQuery('.productInfoLeft').find('.clearFloat').append('.size');
      };


        $(".stockInfo").after('<div class="add-to-cart"<form><label>Qty</label><span><input class="number-product" type="text" value="1" /></span><button class="submit-basket">Add to basket</button></form>   <p class="desatch-date"></p><div class="clear"></div></div>');
