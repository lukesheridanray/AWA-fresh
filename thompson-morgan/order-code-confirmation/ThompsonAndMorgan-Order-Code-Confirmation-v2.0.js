//function () {
	...
	var exp = (function($) {
		...
		exp.log('Order Code Confirmation - 2.0');
		...
		*/
		...
		.modal-bg {\
			display: none; position: fixed; font-family: Arial, Helvetica, sans-serif; top: 0; right: 0; bottom: 0; left: 0; background: rgba(0,0,0,0.4); z-index: 99999; -webkit-transition: opacity 400ms ease-in; -moz-transition: opacity 400ms ease-in; transition: opacity 400ms ease-in;\
		}\
		#order-confirmation {\
		...
		exp.init = function() {
			...
			$('#keep').click(function(e) {
				...
				// Erase that cookie set by the AJAX call
				setCookie('PROMO_MEDIA_COOKIE', '', -1, '/', '.thompson-morgan.com')
				exp.log("cookieCode erased: " + getCookie("PROMO_MEDIA_COOKIE"));

				// Reset the cookie back to the original
				setCookie("PROMO_MEDIA_COOKIE", cookieCode)
				exp.log("cookieCode keeped: " + getCookie("PROMO_MEDIA_COOKIE"));
