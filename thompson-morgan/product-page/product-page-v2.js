...
exp.vars = {
    ...
    arriveTab:    ' \
        ...
        <strong>specially designed containers which prevent dehydration and protect from \
        knocks and bruises</strong>.</p> \
        <p>On despatch, each plant is examined by hand to ensure they \
        are in optimal condition. We check for:</p> \
        <ul> \
           <li>Size</li> \
        ...
        <p>Delivery staff know they are carrying live plants and take extra care to \
        ensure they arrive safely.</p> \
        <p>Every order comes with a Growing Guide with easy-to-follow care instructions \
        for your plants.</p>',
...
*/
  ...
  if (!window.location.href.match(/\/garden-supplies\//) &&
    ...
    $('#tabbed-panel-3').after(
      '<div style="display: none;" id="tabbed-panel-5" class="tabbed-panel"> \
        <div class="arrive-tab-image"><img alt="How plants are packed" src="' +
            (plant_type == 'Plug' || plant_type == 'Mini-plug' || plant_type == 'Jumbo' || plant_type == 'Postiplugs&reg;' ?
                exp.vars.packPlugImg : exp.vars.packPlantImg) + '" /><span class="caption">Packed for protection</span></div> ' +
        exp.vars.arriveTab +
      '</div>');
    showTab(5);
  }
