...
var exp = (function($) {
	...
	exp.log('Special Offers - v4 - var2(chained)');
	...
	*/
	...
	exp.init = function() {
	    ...
	    function getOfferData(url, id) {
			...
			$.ajax({
			    ...
			    success: function(data) {
			    	...
			    	// Build an object with product data for each product (all 12 products will enter this array even if there are less than 12 products on the page)
			    	...
			    	// Remove any product objects that are empty from the array
			    	var newProductArray = [];
				    ...
				    if (newProductArray.length < 10) {
				    	productsToShow = newProductArray.length;
				    }
				    for (var i = 0; i < productsToShow; i++) {
				    	if (newProductArray[i].save) {
				    		list = list + "<div class='AWA-product'><a class='AWA-url' href='" + newProductArray[i].url + "'><img class='AWA-image' src='" + productArray[i].image + "'></a><a class='AWA-url' href='" + productArray[i].url + "'><div class='AWA-product-name'>" + productArray[i].name + "</div></a><div class='AWA-from'>From " + productArray[i].from + "</div><div class='AWA-save'>SAVE up to " + productArray[i].save + "</div></div>";
				    	} else {
				    		list = list + "<div class='AWA-product'><a class='AWA-url' href='" + newProductArray[i].url + "'><img class='AWA-image' src='" + productArray[i].image + "'></a><a class='AWA-url' href='" + productArray[i].url + "'><div class='AWA-product-name'>" + productArray[i].name + "</div></a><div class='AWA-from'>From " + productArray[i].from + "</div></div>";
				    	}
				    }
				    ...
				    if (itemCount.num > 12) {
				    	...
				    	$(id + " .AWA-offer-box").prepend("<div class='AWA-view-all'><span class='AWA-item-count'>Showing " + productsToShow + " of " + itemCount.num + " items</span></div>");
				    }

				    // Add arrows if there are more than 5 products to scroll through
				    if (newProductArray.length > 5) {
					...
					getOfferData2("http://www.thompson-morgan.com/Buy-4-packs-of-bulbs-get-the-cheapest-free",  "#AWA-autumn-planting");
			    }
			});
		};

	    function getOfferData2(url, id) {
	    	var productArray = [];
	    	var id = id;
			...
			$.ajax({
			    ...
			    success: function(data) {
					...
					getOfferData3("http://www.thompson-morgan.com/value-seed-varieties", "#AWA-seed-special-offers");
			    }
			});
		};

	    function getOfferData3(url, id) {
	    	var productArray = [];
	    	var id = id;
		...
		getOfferData("http://www.thompson-morgan.com/flower-bulb-sale", "#AWA-plant-special-offers");
