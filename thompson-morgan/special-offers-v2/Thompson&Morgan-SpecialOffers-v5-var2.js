...
var exp = (function($) {
	...
	exp.log('Special Offers - v5 - var2');
	...
	*/
	...
	exp.init = function() {
	    ...
	    function getOfferData(url, callback, id) {
			...
			$.ajax({
			    ...
			    success: function(data) {
			    	...
			    	// Execute callback
					callback(newProductArray, itemCount, id);
		...
		// NOTE: setTimeout is needed to stagger the Ajax calls because some server problem was causing the same content to be returned for all 3 requests. It's unknown why this was happening but this staggered approach works.
		// NOTE: IE is having problems with this approach. Test is currently set to async: false as a last resort
		setTimeout(function(){getOfferData("http://www.thompson-morgan.com/flower-bulb-sale", bestSellerCallback, "#AWA-plant-special-offers")}, 0);
		setTimeout(function(){getOfferData("http://www.thompson-morgan.com/Buy-4-packs-of-bulbs-get-the-cheapest-free", bestSellerCallback, "#AWA-autumn-planting")}, 1000);
		setTimeout(function(){getOfferData("http://www.thompson-morgan.com/value-seed-varieties", bestSellerCallback, "#AWA-seed-special-offers")}, 2000);
